...
# Funciones para la definición y entrenamiento de la red neuronal
class NeuralNetwork(nn.Module):
    ...
    def fit(self, X_train, y_train, X_val, y_val, num_epochs=1500):
        ...
        mean_val_mae = None
        mean_val_r2 = None
        num_train_batches = int(len(X_train)/self.batch_size)
        for epoch in range(num_epochs):
            ...
            mean_val_mse = total_val_mse 
            mean_val_mae = total_val_mae 
            mean_val_r2 = total_val_r2
...
def main():
    ...
    #k_sizes= [1000, 3000,4000]
    k_sizes= [3000, 5000,10000,30000,50000,100000,130000]
    all_loss_values = []
    all_stats = []
    dataset = utils.qm9_dataset(target=10)
    dataset.load_and_process_data(dir_path=dir_path,name_descriptors='mordred')
    ...
    for k in k_sizes:
        ...
        y_pred, y_test, mean_mse_t, mean_mae_t, mean_r2_t,average_loss = neural_network.test(X_test,y_test)
        print(f"Test Loss:")
        ...
        utils.plot_scatter(y_test, y_pred,k=k,arq_name='NN_mordred')

        residuals = y_test - y_pred
        utils.plot_residuals(y_pred, residuals,k=k,arq_name='NN_mordred')

        # Gráfica de pérdida durante el entrenamiento
    utils.guardar_lista_diccionarios_a_csv(all_stats,arq_name='NN_mordred')
    utils.plot_loss_convergence(all_loss_values, list(map(str, k_sizes)),arq_name='NN_mordred',save_interval=50)
